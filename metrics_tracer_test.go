...
func newTestPrometheusTracer(t *testing.T) (*prometheusTracer, string) {
	...
	tr, err := newPrometheusTracer(logPath, "test-node")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrometheusTracer_E2E(t *testing.T) {
	...
	psubs := getGossipsubsOptFn(ctx, hosts, func(i int, _ host.Host) []Option {
		...
		mt, err := newPrometheusTracer(logPath, hosts[i].ID().String())
		if err != nil {
			t.Fatal(err)
		}
...
func TestPrometheusTracer_NodeID_InMetrics(t *testing.T) {
	const testNodeID = "test-node-abc"
	tr, logPath := newTestPrometheusTracer(t)
	// Override with a tracer that has a specific nodeID
	f, err := os.CreateTemp(t.TempDir(), "metrics-*.log")
	if err != nil {
		t.Fatal(err)
	}
	_ = logPath
	nodeTr, err := newPrometheusTracer(f.Name(), testNodeID)
	if err != nil {
		t.Fatal(err)
	}
	defer nodeTr.Close()
	...
	rpc := makeRPC("test_topic", []byte("data"))
	nodeTr.OnRPCSent(pid, 5*time.Millisecond, rpc, idGen)
	nodeTr.OnRPCReceived(pid, 3*time.Millisecond, rpc, idGen)
	nodeTr.OnPeerRTT(pid, "test_topic", 50*time.Millisecond, "quic", "1.2.3.4:9000")
	nodeTr.OnMeshSize("test_topic", 5)

	// Verify node_id label is present on metrics by collecting them
	bytesSent, err := topicMessageBytesSentMetric.GetMetricWithLabelValues(testNodeID, "test_topic")
	if err != nil {
		t.Fatalf("failed to get metric with node_id label: %v", err)
	}

	dto := &io_prometheus_client.Metric{}
	if err := bytesSent.Write(dto); err != nil {
		t.Fatalf("failed to write metric: %v", err)
	}
	if dto.GetCounter().GetValue() <= 0 {
		t.Fatal("expected bytes sent > 0")
	}

	// Verify mesh size metric has node_id
	meshGauge, err := meshSizeMetric.GetMetricWithLabelValues(testNodeID, "test_topic")
	if err != nil {
		t.Fatalf("failed to get mesh_size metric with node_id label: %v", err)
	}
	dto2 := &io_prometheus_client.Metric{}
	if err := meshGauge.Write(dto2); err != nil {
		t.Fatalf("failed to write metric: %v", err)
	}
	if dto2.GetGauge().GetValue() != 5 {
		t.Fatalf("expected mesh_size=5, got %v", dto2.GetGauge().GetValue())
	}
}
