...
func TestScoreRetention(t *testing.T) {
	...
	// check that the tracer has the correct values
	// as peer A has been removed and scores has been refreshed, the peer stats should not be tracked.
	_, ok := tracer.GetPeerStats(peerA)
	if ok {
		t.Fatal("expected peer stats not to be tracked")
	}
}
